...
//     // const totalArticles = await NewsModel.countDocuments({});
//     const ArticlesAre = await NewsModel.find({});
//     let total=0
//     ArticlesAre.map((article)=>{
//         total+=article.articles.length;
//     });
//     res.status(200).json({ news: News, totalResults: parseInt(total), message: 'Fetched successfully' });
...
// const NewsapiDataGet = async (req, res) => {
//   try {
//     const { page = 1, pageSize = 50, q, from, to, country, category, sources, domains } = req.query;
//     const skip = (page - 1) * pageSize;

//     const matchConditions = {};
//     if (q) {
//       matchConditions['SearchBox.q'] = { $regex: q, $options: 'i' };
//     }
//     if (from || to) {
//       matchConditions['articles.publishedAt'] = {};
//       if (from) matchConditions['articles.publishedAt'].$gte = new Date(from);
//       if (to) matchConditions['articles.publishedAt'].$lte = new Date(to);
//     }
//     if (country) {
//       matchConditions['SearchBox.country'] = country;
//     }
//     if (category) {
//       matchConditions['SearchBox.category'] = category;
//     }
//     if (sources) {
//       matchConditions['articles.source.name'] = sources;
//     }
//     if (domains) {
//       matchConditions['articles.url'] = { $regex: domains, $options: 'i' };
//     }
...
//       {
//         $group: {
//           _id: null, // Added _id field as null since we want to aggregate all documents into a single group
...
//     const News = await NewsModel.aggregate(aggregationPipeline);

//     const totalArticles = await NewsModel.aggregate([
//       { $unwind: '$articles' },
//       { $match: matchConditions },
//       { $count: 'total' },
//     ]);

//     const totalResults = totalArticles.length > 0 ? totalArticles[0].total : 0;
//     console.log(totalResults,News)

//     res.status(200).json({ news: News, totalResults: totalResults, message: 'Fetched successfully' });
//   } catch (error) {
...
//     if (q) {
//       regexConditions.push({ $regexMatch: { input: '$$article.q', regex: new RegExp(q, 'i') } });
//     }
//     if (from) {
//       regexConditions.push({ $gte: ['$$article.publishedAt', new Date(from)] });
//     }
//     if (to) {
//       regexConditions.push({ $lte: ['$$article.publishedAt', new Date(to)] });
//     }
//     if (country) {
//       regexConditions.push({ $eq: ['$$article.country', country] });
//     }
//     if (category) {
//       regexConditions.push({ $eq: ['$$article.category', category] });
//     }
//     if (sources) {
//       regexConditions.push({ $eq: ['$$article.source.name', sources] });
//     }
//     if (domains) {
//       regexConditions.push({ $regexMatch: { input: '$$article.url', regex: new RegExp(domains, 'i') } });
//     }

//     const aggregationPipeline = [
//       { 
//         $match: {
//           ...(q && { 'SearchBox.q': { $regex: new RegExp(q, 'i') } })
//         }
//       },
//       { 
//         $project: {
...
//               as: 'article',
//               cond: {
//                 $and: regexConditions.length ? regexConditions : [{}]
//               }
//             }
//           }
//         }
//       },
//       { $unwind: '$matchedArticles' },
//       { $replaceRoot: { newRoot: '$matchedArticles' } },
//       { $skip: skip },
//       { $limit: parseInt(pageSize) }
//     ];

//     console.log(aggregationPipeline,regexConditions)
//     const News = await NewsModel.aggregate(aggregationPipeline);

//     const totalArticlesPipeline = [
//       { 
//         $match: {
//           ...(q && { 'SearchBox.q': { $regex: new RegExp(q, 'i') } })
//         }
...
//     const totalArticlesResult = await NewsModel.aggregate(totalArticlesPipeline);
//     const totalResults = totalArticlesResult.length > 0 ? totalArticlesResult[0].total : 0;
...
//           cond: {
//             $and: [
//               { $regexMatch: { input: '$$article.title', regex: new RegExp(q, 'i') } },
...
//   try {
//     const articles = await NewsModel.aggregate(pipeline);
//     res.json({ news: articles, totalResults: articles.length });
//   } catch (err) {
//     res.status(500).json({ error: err.message });
//   }
// };
...
//           cond: {
//             $and: [
//               // / yha match SearchBox.q se krvana
//               // { $regexMatch: { input: '$$article.title', regex: new RegExp(q, 'i') } },
//               { $gte: ['$$article.publishedAt', new Date(from)] },
//               { $lte: ['$$article.publishedAt', new Date(to)] }
//             ]
...
//   { $count: 'total' }
// ];
// // console.log(matchStage,pipeline)

// try {
//   const articles = await NewsModel.aggregate(pipeline);
//   const articlesCount = await NewsModel.aggregate(pipelinecount);

//   console.log(articles.length, articlesCount[0].total)
//   // console.log(articles)
...
//   const { page = 1, pageSize = 50, q, from, to, sortBy, country, category, sources, domains } = req.query;
//   const skip = (page - 1) * pageSize;
//   // console.log("pages", page, skip, domains, q)

//   const matchStage = {}
//   if (q) {
//     matchStage['SearchBox.q'] = { $regex: q, $options: 'i' };
//   }
//   if (domains) {
...
//           cond: {
//             $and: [
//               // ...(q ? [{ $regexMatch: { input: '$$article.title', regex: new RegExp(q, 'i') } }] : []),
//               { $gte: ['$$article.publishedAt', new Date(from)] },
//               { $lte: ['$$article.publishedAt', new Date(to)] },
//               // ...(from ? [{ $gte: ['$$article.publishedAt', new Date(from)] }] : []),
//               // ...(to ? [{ $lte: ['$$article.publishedAt', new Date(to)] }] : []),
//             ]
//           }
//         }
//       },
...
//               // { $regexMatch: { input: '$$item.domains', regex: new RegExp(domains, 'i') } },
//               // ...(domains ? [{ $eq: ['$$item.domains', domains] }] : []),
//             ]
//           }
//         }
//       }
...
//   const matchStage = {};
//   if (searchTerm) {
//     matchStage['SearchBox.q'] = { $regex: searchTerm, $options: 'i' };
//   }
...
//           as: 'item',
//           cond: {
//             $regexMatch: { input: '$$item.q', regex: new RegExp(searchTerm, 'i') }
//           }
//         }
//       }
//     }
