...
function Profile() {
  ...
  // for profile ----
    ...
    useEffect(() => {
      const darkModeSetting = localStorage.getItem("dark-mode") === "enabled";
  ...
  useEffect(() => {
    const fetchProfileData = async () => {
      ...
      try {
        ...
        navigate.push("/auth?startwith=signIn")
        console.error(error);
    ...
    <>
      <header className="header">
        <section className="flex">
          <Link href="#" className="logo">NewsApi</Link>
      ...
      <div className="container">
        <div className={`left-container side-bar ${isSideBarActive ? 'active' : ''}`}>
          ...
          <nav className="navbar">
            <Link href="/index"><i className="fas fa-home"></i><span>home</span></Link>
            <Link href="/NewsApis"><i className="fas fa-chalkboard-user"></i><span>Newsapis</span></Link>
            <Link href="/DataBaseData"><i className="fas fa-database"></i><span>Database</span></Link>
            <Link href="/Profile"><i className="fas fa-graduation-cap"></i><span>profile</span></Link>
            <Link href="/contact"><i className="fas fa-headset"></i><span>contact us</span></Link>
          </nav>
        </div>
        <div className="right-container">
          <section className="about">
            <div className="row">
              ...
              {IfAdmin_1 ? <Middlerow /> : user.admin==='0' ? <Link style={{border:'2px solid #c3cbff',padding:'10px',fontSize:'12px'}} href="/contact"><i className="fas fa-envelope"></i><span>Send an email to become an admin</span></Link>:<span style={{border:'2px solid #c3cbff',padding:'10px',fontSize:'12px'}}>click on the "Admin" button above.</span>}
            </div>
